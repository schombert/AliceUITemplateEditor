...
plutovg_paint_t* plutovg_paint_create_subpx_texture(plutovg_surface_t* surface_lx_ly, plutovg_surface_t* surface_sx_sy, plutovg_surface_t* surface_lx_sy, plutovg_surface_t* surface_sx_ly, float opacity, const plutovg_matrix_t* matrix, float real_x_dim, float real_y_dim) {
        ...
        texture->surface_sx_sy = plutovg_surface_reference(surface_sx_sy);
        texture->surface_lx_sy = plutovg_surface_reference(surface_lx_sy);
        texture->surface_sx_ly = plutovg_surface_reference(surface_sx_ly);
        texture->real_x_extent = real_x_dim;
        texture->real_y_extent = real_y_dim;
        return &texture->base;
}
...
void plutovg_paint_destroy(plutovg_paint_t* paint)
{
    if(plutovg_destroy_reference(paint)) {
        ...
        } else if(paint->type == PLUTOVG_PAIN_TYPE_SUBPXTEXTURE) {
                plutovg_subpx_texture_paint_t* texture = (plutovg_subpx_texture_paint_t*)(paint);
                plutovg_surface_destroy(texture->surface_lx_ly);
                plutovg_surface_destroy(texture->surface_sx_sy);
                plutovg_surface_destroy(texture->surface_lx_sy);
                plutovg_surface_destroy(texture->surface_sx_ly);
        }
